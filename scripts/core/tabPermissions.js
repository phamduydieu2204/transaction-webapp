...
export function parseTabPermissions(tabNhinThay) {
  ...
  // '🔍 Parsed permissions array:', permissions;

  // Check for "tất cả" first (gives all permissions)
  if (permissions.includes('tất cả')) {
    // '✅ Found "tất cả" permission - granting all tabs';
  ...
  // '✅ Final parsed tab permissions:', result;
...
export function canAccessTab(tabId) {
  ...
  // `🔐 Tab access check: ${tabId} = ${canAccess ? '✅ ALLOWED' : '❌ DENIED'}`;
...
export function getDefaultAllowedTab() {
  ...
  // `🎯 Default allowed tab: ${defaultTab}`;
  return defaultTab;
}
...
export function filterAllowedTabs(allTabs) {
  ...
  // '🔍 Filtered allowed tabs:', filtered.map(t => t.id);
  return filtered;
}
...
export function validateTabAccess(requestedTab) {
  // `🔍 Validating access to tab: ${requestedTab}`;
  ...
  if (canAccessTab(requestedTab)) {
    // `✅ Access granted to tab: ${requestedTab}`;
    return requestedTab;
  } else {
    const defaultTab = getDefaultAllowedTab();
...
export function initializeTabPermissions() {
  // '🔐 Initializing tab permissions system...';
  ...
  // '✅ Tab permissions system initialized';
  return true;
}